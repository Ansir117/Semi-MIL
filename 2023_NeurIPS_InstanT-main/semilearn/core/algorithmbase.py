...
class AlgorithmBase:
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        # 添加AUC计算（仅对二分类有效）
        auc_score = None
        if self.num_classes == 2 and len(np.unique(y_true)) > 1:
            try:
                from sklearn.metrics import roc_auc_score
                # 从logits计算概率，取正类概率
                y_probs = torch.softmax(torch.from_numpy(y_logits), dim=1)[:, 1].numpy()
                ...
                self.print_fn(f"AUC calculation failed: {e}")
                auc_score = 0.0
        ...
        if auc_score is not None:
            eval_dict[eval_dest + '/auc'] = auc_score
        #################
        if return_logits:
