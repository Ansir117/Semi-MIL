...
# ==================== 数据集 ====================
# 尝试导入独立的数据集文件，如果不存在则使用内置版本
try:
    from dataset_CAMELYON16_E2E import CAMELYON16_E2E as CAMELYON16_Image

    print("[INFO] 使用外部数据集文件: dataset_CAMELYON16_E2E.py")
except ImportError:
    # 内置简化版数据集
    class CAMELYON16_Image(torch.utils.data.Dataset):
        ...
        def __init__(self, root_dir, train=True, transform=None, return_bag=False,
                     max_bag_size=100, ssl_mode=False, labeled_ratio=0.2, ssl_seed=42):
            import glob
        ...
        def __getitem__(self, index):
            if self.return_bag:
                ...
                return images, [torch.tensor(self.patch_labels[idx_patches]),
                                ...
                                index, self.slide_names[idx_patches[0]]], index
            else:
                ...
                return img, [torch.tensor(self.patch_labels[index]), torch.tensor(self.slide_labels[index]),
                             torch.tensor(self.slide_indices[index]), self.slide_names[index]], index
...
class E2EHybridTrainer:
        ...
        for batch_idx, batch in enumerate(tqdm(loader, desc='SSL')):
            ...
            if self.use_ssl_dataset and isinstance(batch, dict):
                ...
                if labeled_mask.sum() > 0:
                    ...
                    loss_labeled = criterion(ssl_logits_lb, labels_lb.long()).mean()
    ...
    def _train_student_hybrid(self, epoch):
        ...
        for batch_idx, (images, labels, _) in enumerate(tqdm(self.train_patch_loader, desc='Student')):
            ...
            # 加权BCE损失
            loss = -torch.mean(
                0.1 * (1 - hybrid_pseudo) * torch.log(student_prob[:, 0] + 1e-5) +
                0.9 * hybrid_pseudo * torch.log(student_prob[:, 1] + 1e-5)
            )
...
def main():
    ...
    try:
        from dataset_CAMELYON16_E2E import create_ssl_split, SSLPatchDataset
