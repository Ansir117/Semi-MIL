...
class InstanTDual(AlgorithmBase):
    ...
    def train_step(self, x_lb, y_lb, idx_ulb, x_ulb_w, x_ulb_s):
        ...
        if self.it >= self.teacher_warmup:
            # Teacher: 用所有labeled patches作为一个bag训练
            # 关键修复：转换为float32避免dtype不匹配
            with torch.no_grad():
