...
class CervicalCaner_16(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir="",
        ...
        patch_label_map = self._load_patch_label_file(self.patch_label_file)
        cnt_slide = 0
        cnt_patch = 0
        for i in tqdm(all_slides, ascii=True, desc='preload data'):
            ...
            for j in patches_from_slide_i:
                ...
                patch_rel_path = os.path.join(slide_name, j)
                patch_abs_path = os.path.join(i, j)
                patch_label, patch_has_gt = self._resolve_patch_label(
                    patch_name=j,
                    patch_rel_path=patch_rel_path,
                    patch_abs_path=patch_abs_path,
                    patch_label_map=patch_label_map,
                )
        ...
        self.patch_label_has_gt = np.array(self.patch_label_has_gt, dtype=np.long)
        ...
        num_has_gt = int(self.patch_label_has_gt.sum())
        if num_has_gt > 0:
            gt_pos_ratio = float(self.patch_label[self.patch_label_has_gt == 1].mean())
            gt_info = "real_patch_labels: {} (pos_ratio={:.4f})".format(num_has_gt, gt_pos_ratio)
        else:
            gt_info = "real_patch_labels: 0"
        print("[DATA INFO] num_slide is {}; num_patches is {}\npos_patch_ratio is unknown".format(
            self.num_slides, self.num_patches))
        print("[DATA INFO] {}".format(gt_info))
        print("")
    ...
    def _load_patch_label_file(patch_label_file):
        ...
        if not os.path.exists(patch_label_file):
            raise FileNotFoundError("patch_label_file not found: {}".format(patch_label_file))
    ...
    @staticmethod
    def _resolve_patch_label(patch_name, patch_rel_path, patch_abs_path, patch_label_map):
        if len(patch_label_map) == 0:
            return 0, 0

        candidate_keys = [
            os.path.normpath(patch_abs_path),
            os.path.normpath(patch_rel_path),
            os.path.normpath(patch_name),
        ]
        for candidate in candidate_keys:
            if candidate in patch_label_map:
                return int(patch_label_map[candidate]), 1
        return 0, 0
    ...
    def __getitem__(self, index):
        if self.return_bag:
            ...
            patch_labels_has_gt = self.patch_label_has_gt[idx_patch_from_slide_i]
            ...
            if self.patch_corresponding_slide_index[idx_patch_from_slide_i].max() != self.patch_corresponding_slide_index[idx_patch_from_slide_i].min():
                raise
            return bag, [patch_labels, slide_label, slide_index, slide_name, patch_labels_has_gt], index
        else:
            if self.preload:
            ...
            patch_label_has_gt = self.patch_label_has_gt[index]
            ...
            return patch_image, [patch_label, patch_corresponding_slide_label, patch_corresponding_slide_index,
                                 patch_corresponding_slide_name, patch_label_has_gt], index
