...
class FeatureHybridTrainer:
    ...
    def _evaluate(self, epoch):
        ...
        if len(all_gt) > 0:
            gt = torch.cat(all_gt)
            self.writer.add_scalar("auc/test_student", cal_auc(gt, torch.cat(all_student)), epoch)
            self.writer.add_scalar("auc/test_ssl", cal_auc(gt, torch.cat(all_ssl)), epoch)
            self.writer.add_scalar("auc/test_teacher", cal_auc(gt, torch.cat(all_teacher)), epoch)
