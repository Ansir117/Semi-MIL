# camelyon16.py - 最终修复版，解决评估数据格式问题
...
class PathBasedDataset(Dataset):
    """基于路径的数据集，返回USB框架期望的字典格式，避免参数冲突"""
    ...
    def __getitem__(self, idx):
        ...
        if self.is_ulb:
            ...
            return {
                ...
                'x_ulb_s': img_strong,
                'y_ulb': target
                # 注意：不返回path和slide_name，避免process_batch冲突
            }
        else:
            # 有标签数据：返回标准USB格式
            if self.transform_weak:
...
class EvalDatasetWithSlideInfo(Dataset):
    """
    专门用于评估的数据集，返回USB框架期望的字典格式，同时包含slide信息用于聚合
    """
    ...
    def __getitem__(self, idx):
        ...
        # 返回USB框架期望的字典格式，但包含slide信息
        return {
            'x_lb': image,
            'y_lb': target,
            # 额外的slide信息，用于聚合但不影响标准评估流程
            'slide_path': path,
...
def get_camelyon16(args, alg='instant', dataset='camelyon16', num_labels=500, num_classes=2,
    ...
    """
    获取CAMELYON16数据集的SSL划分 - 最终修复版，解决所有格式冲突问题
    """
