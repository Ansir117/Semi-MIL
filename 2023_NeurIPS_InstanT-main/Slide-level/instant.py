...
# 添加slide聚合支持
try:
    from slide_aggregation_utils import SlideAggregator, compute_slide_level_metrics
except ImportError:
    print("Warning: slide_aggregation_utils not found. Slide-level evaluation will be disabled.")
    SlideAggregator = None
    compute_slide_level_metrics = None
...
class InstanT(AlgorithmBase):
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        # ----------- Patch for missing attributes in hooks ------------
        self.average_thres = 0.0
        ...
        self.class_T = torch.ones((self.num_classes, self.num_classes)).cuda(self.gpu)
        self.estimator = nn.Identity()  # Dummy default to avoid crash
        # --------------------------------------------------------------

        # ----------- Slide聚合支持 ------------
        self.slide_aggregation_method = getattr(args, 'slide_aggregation_method', 'max')
        self.save_slide_results = getattr(args, 'save_slide_results', False)
        self.slide_aggregator = None
        if SlideAggregator is not None:
            self.slide_aggregator = SlideAggregator(
                aggregation_method=self.slide_aggregation_method,
                logger=logger
            )
        # ----------------------------------------------
    ...
    def train_step(self, x_lb, y_lb, idx_ulb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            mask = self.call_hook("masking", "MaskingHook", logits_x_lb=probs_x_lb, logits_x_ulb=probs_x_ulb_w,
                                  x_ulb_w=x_ulb_w, softmax_x_lb=False, softmax_x_ulb=False)
    ...
    def evaluate_slide_level(self, data_loader=None):
        """
        评估slide级别性能 - 新增方法
        """
        if self.slide_aggregator is None or data_loader is None:
            self.print_fn("Slide聚合器未初始化或数据加载器为空，跳过slide级别评估")
            return None

        eval_model = self.ema_model if hasattr(self, 'ema_model') and self.ema_model is not None else self.model
        eval_model.eval()

        # 重置聚合器
        self.slide_aggregator.reset()

        try:
            with torch.no_grad():
                for batch in data_loader:
                    if isinstance(batch, dict):
                        inputs = batch.get('x_lb', batch.get('x', batch.get('image'))).cuda(self.gpu)
                        labels = batch.get('y_lb', batch.get('y', batch.get('target', batch.get('label')))).cuda(
                            self.gpu)
                        paths = batch.get('path', batch.get('paths', None))
                    elif len(batch) >= 4:  # (image, label, path, slide_name)
                        inputs, labels, paths = batch[0].cuda(self.gpu), batch[1].cuda(self.gpu), batch[2]
                    else:
                        inputs, labels = batch[0].cuda(self.gpu), batch[1].cuda(self.gpu)
                        paths = None
                    ...
                    if paths is not None:
                        self.slide_aggregator.add_batch(probs, labels, paths)

            # 计算slide级别指标
            slide_results = self.slide_aggregator.compute_slide_metrics()

            if slide_results and slide_results['metrics']:
                slide_metrics = slide_results['metrics']
                self.print_fn(f"Slide级别评估结果:")
                self.print_fn(f"  AUC: {slide_metrics.get('auc', 'N/A')}")
                self.print_fn(f"  准确率: {slide_metrics.get('accuracy', 'N/A')}")
                self.print_fn(f"  slide数量: {slide_metrics.get('num_slides', 'N/A')}")
                self.print_fn(f"  聚合方法: {self.slide_aggregation_method}")
                ...
                if hasattr(self, 'results_dict'):
                    self.results_dict.update({
                        ...
                        'slide_aggregation_method': self.slide_aggregation_method,
                        'num_slides': slide_metrics.get('num_slides')
                    })

                # 保存详细结果
                if self.save_slide_results and slide_results:
                    import os
                    from slide_aggregation_utils import save_slide_results
                    save_path = os.path.join(self.save_path, 'slide_results.json')
                    save_slide_results(
                ...
                self.print_fn("无法计算slide级别指标")
                return None

        except Exception as e:
            self.print_fn(f"Slide级别评估出错: {e}")
            import traceback
            traceback.print_exc()
            return None
        finally:
    ...
    def get_save_dict(self):
        ...
        # 保存slide聚合相关配置
        save_dict['slide_aggregation_method'] = self.slide_aggregation_method
        save_dict['save_slide_results'] = self.save_slide_results
    ...
    def load_model(self, load_path):
        ...
        # 加载slide聚合相关配置
        self.slide_aggregation_method = checkpoint.get('slide_aggregation_method', 'max')
        self.save_slide_results = checkpoint.get('save_slide_results', False)
    ...
    def get_argument():
        return [
            ...
            # 新增slide聚合参数
            SSL_Argument('--slide_aggregation_method', str, 'max',
                         'method for slide-level aggregation: max, mean, top_k'),
            SSL_Argument('--save_slide_results', str2bool, False, 'save detailed slide-level results'),
        ]