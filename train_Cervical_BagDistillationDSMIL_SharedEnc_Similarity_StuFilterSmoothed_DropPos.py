...
from models.resnetv1 import resnet_NCT_Encoder, teacher_DSMIL_head, student_head
...
class Optimizer:
    def __init__(self, model_encoder, model_teacherHead, model_studentHead,
                 ...
                 student_ssl_mode='weno', inst_weight_supervised=0.7, inst_conf_threshold=0.6):
        ...
        self.student_ssl_mode = student_ssl_mode
        self.inst_weight_supervised = inst_weight_supervised
        self.inst_conf_threshold = inst_conf_threshold
    ...
    def optimize_student(self, epoch):
        ...
        for iter, (data, label, selected) in enumerate(tqdm(loader, desc='Student training')):
            ...
            gt_patch_label = label[0].float()
            if len(label) > 4:
                gt_patch_has_label = label[4].float()
            else:
                gt_patch_has_label = torch.zeros_like(gt_patch_label).float()
            ...
            # cal loss
            if self.student_ssl_mode == 'instant':
                target = pseudo_instance_label.clone()
                pseudo_conf = torch.abs(pseudo_instance_label - 0.5) * 2.0
                pseudo_mask = (pseudo_conf >= self.inst_conf_threshold).float()
                if gt_patch_has_label.sum() == 0:
                    pseudo_mask = torch.ones_like(pseudo_mask)
                target[gt_patch_has_label > 0] = gt_patch_label[gt_patch_has_label > 0]
                sample_weight = (1.0 - self.inst_weight_supervised) * pseudo_mask
                sample_weight[gt_patch_has_label > 0] = self.inst_weight_supervised
                loss_per_sample = -1.0 * ((1-target) * torch.log(patch_prediction[:, 0] + 1e-5) +
                                          target * torch.log(patch_prediction[:, 1] + 1e-5))
                loss_student = (loss_per_sample * sample_weight).sum() / (sample_weight.sum() + 1e-6)
                if niter % self.log_period == 0:
                    self.writer.add_scalar('train_supervised_patch_ratio', gt_patch_has_label.mean().item(), niter)
                    self.writer.add_scalar('train_confident_pseudo_ratio', pseudo_mask.mean().item(), niter)
            else:
                loss_student = -1. * torch.mean(
                    self.stu_loss_weight_neg * (1-pseudo_instance_label) * torch.log(patch_prediction[:, 0] + 1e-5) +
        ...
        if self.student_ssl_mode == 'instant' and hasattr(loader.dataset, 'patch_label_has_gt'):
            patch_label_has_gt = torch.tensor(loader.dataset.patch_label_has_gt).to(self.dev).reshape(-1)
            if patch_label_has_gt.sum() > 0:
                instance_auc_ByStudent = utliz.cal_auc(
                    patch_label_gt.reshape(-1)[patch_label_has_gt == 1],
                    patch_label_pred.reshape(-1)[patch_label_has_gt == 1]
                )
            else:
...
def get_parser():
    ...
    parser.add_argument('--student_ssl_mode', default='weno', type=str, choices=['weno', 'instant'],
                        help='weno uses pseudo labels only; instant mixes real patch labels with pseudo labels')
    parser.add_argument('--inst_weight_supervised', default=0.7, type=float,
                        help='loss weight assigned to real patch labels when student_ssl_mode=instant')
    parser.add_argument('--inst_conf_threshold', default=0.6, type=float,
                        help='confidence threshold for pseudo labels when student_ssl_mode=instant')
    parser.add_argument('--patch_label_file', default='', type=str,
                        help='optional csv/tsv file containing real patch labels: patch_path,label')
...
if __name__ == "__main__":
    ...
    name = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")+"_%s" % args.comment.replace('/', '_') + \
           "_Seed{}_Bs{}_lr{}_Downsample{}_PLPostProcessBy{}_StuFilterType{}_smoothE{}_weightN{}_stuOptP{}_StuSSL{}".format(
    ...
    # Setup loaders
    train_ds_return_instance = CervicalCaner_16(
        train=True, transform=None, downsample=args.dataset_downsampling,
        preload=True, return_bag=False, patch_label_file=args.patch_label_file
    )
    train_ds_return_bag = copy.deepcopy(train_ds_return_instance)
    train_ds_return_bag.return_bag = True
    val_ds_return_instance = CervicalCaner_16(
        train=False, transform=None, downsample=args.dataset_downsampling,
        preload=True, return_bag=False, patch_label_file=args.patch_label_file
    )
    val_ds_return_bag = CervicalCaner_16(
        train=False, transform=None, downsample=args.dataset_downsampling,
        preload=True, return_bag=True, patch_label_file=args.patch_label_file
    )
    ...
    o = Optimizer(model_encoder=model_encoder, model_teacherHead=model_teacherHead, model_studentHead=model_studentHead,
                  ...
                  student_ssl_mode=args.student_ssl_mode, inst_weight_supervised=args.inst_weight_supervised,
                  inst_conf_threshold=args.inst_conf_threshold)
    # Optimize
    o.optimize()